 ...
 */
export function formatTranscriptAsText(segments: TranscriptionSegment[]): string {
  return segments
    .map(
      (seg) =>
 ...
 */
export function isValidTranscriptionResult(data: unknown): data is TranscriptionResult {
  ...
  return obj.segments.every((segment: unknown) => {
    if (segment === null || segment === undefined || typeof segment !== 'object') {
      return false;
    }
