...
export function useGlassesCamera(): UseGlassesCameraReturn {
  ...
  const initializeCamera = useCallback(async (lowPowerMode: boolean = false) => {
    setError(null);
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : 'Failed to initialize camera';
  ...
  const captureImage = useCallback(async () => {
    ...
    if (!isReady && wasInitializedRef.current) {
      ...
      try {
        ...
        const errorMessage = e instanceof Error ? e.message : 'Failed to reinitialize camera';
        setError(errorMessage);
        return;
      }
    } else if (!isReady) {
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : 'Failed to capture image';
      setError(errorMessage);
      setIsCapturing(false);
    }
  }, [isReady, isCapturing]);
  ...
  const releaseCamera = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : 'Failed to release camera';
      setError(errorMessage);
    }
  }, []);
