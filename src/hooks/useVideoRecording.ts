...
export function useVideoRecording(): UseVideoRecordingReturn {
	...
	useEffect(() => {
		...
		const stateSub = service.onRecordingStateChanged(
	...
	const transcribe = useCallback(async (language: string = "en") => {
		...
		try {
			const result: TranscriptionResponse =
				await serviceRef.current.sendRecordingForTranscription(language);
	...
	const downloadTranscript = useCallback(async () => {
		...
		try {
			...
			if (!shared) {
				...
				Alert.alert(
					"Copied",
					"Sharing unavailable — transcript copied to clipboard.",
				);
			}
		} catch (error) {
