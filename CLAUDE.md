...
|---------|---------------|
| Agora token server | `https://agora-token.spex-remote.workers.dev/` (handles `/token`, `/ws/`, `/heartbeat`, `/leave`, `/viewers`, `/report-error`) |
| Web demo | `https://spex-demo.pages.dev` (Cloudflare Pages static site) |
| Web viewer | `https://spex-viewer.pages.dev/view/{channelId}` |
| Production backend | `https://spex-backend.onrender.com` (Render free tier — cold starts after 15min idle) |
