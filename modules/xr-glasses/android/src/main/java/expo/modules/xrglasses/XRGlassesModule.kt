...
class XRGlassesModule : Module() {
    ...
    @Suppress("LongMethod", "CyclomaticComplexMethod") // Expo module DSL — defines entire JS API surface
    override fun definition() =
        ModuleDefinition {
            ...
            Events(
                ...
                "onImageCaptured",
                "onCameraError",
                // Camera ready state changes
                "onCameraStateChanged",
                // Remote View streaming events (from GlassesActivity via broadcast)
                "onStreamStarted",
                "onStreamStopped",
                "onStreamError",
                // Viewer count/info changed
                "onViewerUpdate",
                // Camera source changed (phone vs glasses)
                "onStreamCameraSourceChanged",
                // Video recording events (state: idle, recording, stopping, stopped)
            ...
            AsyncFunction("getParkingTimerState") { promise: Promise ->
                scope.launch {
                    try {
                        val state =
                            glassesService?.getParkingTimerState() ?: mapOf(
                                "isActive" to false,
                                "remainingMs" to 0L,
                                "endTime" to 0L,
